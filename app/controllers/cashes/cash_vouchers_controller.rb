class Cashes::CashVouchersController < ApplicationController
  ...
  before_action :set_cash_voucher, only: [:show, :edit, :update, :destroy]
  ...
  def create
    @cash_voucher = @cash.cash_vouchers.new(
      cash_voucher_params.merge(user: current_user)
    )
  ...
  private
    ...
    def cash_voucher_params
      params.require(:cash_voucher).permit :kind, :amount, :owner_id, :owner_type, :comment,
        :cash_invoice_id, :lock_version
    end
end
